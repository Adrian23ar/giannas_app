...
<template>
  <Transition name="fade">
    <div v-if="show" @click.self="emit('close')"
      class="fixed inset-0 bg-black/60 flex justify-center items-center z-50 p-4 pt-20">
      <div v-if="product"
        class="bg-pink-100 rounded-2xl shadow-xl w-full max-w-4xl max-h-[calc(100vh-7rem)] overflow-y-auto flex flex-col md:flex-row gap-6 md:gap-8 px-6 py-11 sm:p-10 relative">
        ...
        <div class="w-full md:w-1/2 md:order-last order-first">
          <div class="bg-white p-1 rounded-2xl">
            <img :src="product.foto_url" :alt="product.nombre" class="w-full object-cover rounded-xl aspect-square">
            </div>
        </div>

        <div class="w-full md:w-1/2 flex flex-col md:order-first order-last">
          <h2 class="text-2xl md:text-3xl font-bold text-brand-morado">{{ product.nombre }}</h2>
